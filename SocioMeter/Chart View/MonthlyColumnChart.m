...
@implementation MonthlyColumnChart
...
- (void)setDelegate:(id<MonthlyColumnChartDelegate>)delegate
{
...
- (void)setDataSource:(id<MonthlyColumnChartDataSource>)dataSource
{
        ...
        
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 11; i++)
                {
                    ...
                    EColumnChartLabel *eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(-1 * Y_COORDINATE_LABEL_WIDTH, -heightGap / 2.0 + heightGap * i, Y_COORDINATE_LABEL_WIDTH, heightGap)];
                    [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
                    eColumnChartLabel.text = [[NSString stringWithFormat:@"%.1d ", 10 *  i] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit];;
                    NSLog(@" horizantal count==%@ ",[NSString stringWithFormat:@"%.1d ", 10 *  i]  );
...
- (void)initData
{
    ...
    _minColumnColor = EGrey;
    _maxColumnColor =  EMinValueColor;
    _normalColumnColor = EMaxValueColor;
...
- (void)reloadData
{
    ...
    float widthOfTheColumnShouldBe = (self.frame.size.width / 30 );
    
    
    // float widthOfTheColumnShouldBe = self.frame.size.width / (float)(totalColumnsRequired + (totalColumnsRequired ) );
    ...
    for (int i = 0; i < 30; i++)
    {
        ...
        /** Construct Columns*/
        MonthlyColumn *eColumn = [_eColumns objectForKey: [NSNumber numberWithInteger:currentIndex ]];
        if (nil == eColumn)
        {
            eColumn = [[MonthlyColumn alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe  + (i * widthOfTheColumnShouldBe ), 0, widthOfTheColumnShouldBe/3, self.frame.size.height)];
    ...
    {
        MonthlyColumn *eColumn = [_eColumns objectForKey: [NSNumber numberWithInteger: maxIndex]];
...
- (void)moveLeft
{
    ...
    {
        MonthlyColumn *eColumn = [_eColumns objectForKey:[NSNumber numberWithInteger:_leftMostIndex - i * DIRECTION]];
        MonthlyColumn *nextEColumn = [_eColumns objectForKey:[NSNumber numberWithInteger:_leftMostIndex - (i + 1)  * DIRECTION]];
...
- (void)moveRight
{
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        MonthlyColumn *eColumn = [_eColumns objectForKey:[NSNumber numberWithInteger:_rightMostIndex + i * DIRECTION]];
        MonthlyColumn *nextEColumn = [_eColumns objectForKey:[NSNumber numberWithInteger:_rightMostIndex + (i + 1) * DIRECTION]];
...
- (void)eColumnTaped:(MonthlyColumn *)eColumn
{
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (MonthlyColumn *view in _eColumns.objectEnumerator)
    {
